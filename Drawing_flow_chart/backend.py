...
def view_unixoid(filepath):
    ...
    subprocess.Popen(['xdg-open', filepath])      # 打开文件


    #########################################################################
    # subprocess.Popen(['pkill -f', filepath])      # 打开文件
    # import time
    # time.sleep(3)
    # subprocess.Popen(['killall -9 %s'%filepath])      # 打开文件
    # a = os.getpid(filepath)
    # print(a)
    #from subprocess import check_output

    # print(map(int, check_output(["pidof", filepath]).split()))
    #########################################################################
    # import os, signal,time

    # out = os.popen("ps aux | grep %s"%filepath).read()
